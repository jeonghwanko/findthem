...
describe('rateLimit', () => {
  ...
  describe('제한 이내 요청', () => {
    ...
    it('max 횟수 이내의 요청은 모두 next()를 호출한다', async () => {
  ...
  describe('제한 초과 요청', () => {
    it('max를 초과하면 429 응답을 반환한다', async () => {
    ...
    it('제한 초과 시 next()를 호출하지 않는다', async () => {
      const limiter = rateLimit({ windowMs: 60_000, max: 1 });
      const ip = uniqueIp();

      // 1번 통과
      await limiter(createMockReq(ip), createMockRes().res, vi.fn());

      // 2번째 초과
      const next = vi.fn();
      await limiter(createMockReq(ip), createMockRes().res, next);

      expect(next).not.toHaveBeenCalled();
    });

    it('기본 에러 메시지로 응답한다', async () => {
      const limiter = rateLimit({ windowMs: 60_000, max: 1 });
      const ip = uniqueIp();
      ...
      await limiter(createMockReq(ip), res, vi.fn());
    ...
    it('커스텀 message 옵션이 에러 응답에 포함된다', async () => {
  ...
  describe('IP별 독립 카운팅', () => {
    it('서로 다른 IP는 각자 독립적으로 카운팅된다', async () => {
      const limiter = rateLimit({ windowMs: 60_000, max: 1 });
      const ip1 = uniqueIp();
      const ip2 = uniqueIp();
      ...
      await limiter(createMockReq(ip1), createMockRes().res, next1);
      await limiter(createMockReq(ip2), createMockRes().res, next2);

      expect(next1).toHaveBeenCalledOnce();
      expect(next2).toHaveBeenCalledOnce();
    });

    it('IP1이 제한을 초과해도 IP2는 정상 통과한다', async () => {
      const limiter = rateLimit({ windowMs: 60_000, max: 1 });
      const ip1 = uniqueIp();
      const ip2 = uniqueIp();

      // ip1 통과 후 초과
      await limiter(createMockReq(ip1), createMockRes().res, vi.fn());
      const blockedNext = vi.fn();
      await limiter(createMockReq(ip1), createMockRes().res, blockedNext);
      expect(blockedNext).not.toHaveBeenCalled();

      // ip2는 독립적으로 통과
      const ip2Next = vi.fn();
      await limiter(createMockReq(ip2), createMockRes().res, ip2Next);
      expect(ip2Next).toHaveBeenCalledOnce();
    });
  });

  describe('req.ip가 없는 경우', () => {
    it('ip가 undefined이면 unknown 키로 처리하고 next()를 호출한다', async () => {
