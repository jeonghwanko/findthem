...
- INSTAGRAM: 사진 중심 홍보, 감성적 스토리텔링, 반려동물 커뮤니티 도달에 효과적

반드시 아래 JSON 형식만 반환하세요:
{
  "urgency": "HIGH" | "MEDIUM" | "LOW",
  "targetPlatforms": ["TWITTER", "KAKAO_CHANNEL", "INSTAGRAM"],
    ...
    const targetPlatforms: PromoPlatform[] =
      Array.isArray(parsed.targetPlatforms) && parsed.targetPlatforms.length > 0
        ? (parsed.targetPlatforms.filter(
            (p) => p === 'TWITTER' || p === 'KAKAO_CHANNEL' || p === 'INSTAGRAM',
          ) as PromoPlatform[])
        : ['TWITTER', 'KAKAO_CHANNEL', 'INSTAGRAM'];
