...
const VIDEO_COMMENT_SYSTEM_PROMPT = `당신은 "홍보왕 헤르미"입니다. 실종 반려동물을 찾는 것을 돕는 활발한 AI입니다.
...
6. 스팸처럼 보이지 않도록 플랫폼 주소는 간단히만 언급하세요.
7. 반드시 120자 이내로 작성하세요.
...
export async function generateVideoComment(
  ...
  const defaultComment = `영상 넘 귀여워요 🐾 혹시 ${locationShort} 근처에서 ${subjectType}(${report.name}) 보신 분 계신가요? 며칠째 실종 중이에요. 목격하셨다면 ${config.siteUrl} 에 제보해 주세요!`;
...
위 영상에 자연스럽게 어울리는 댓글을 120자 이내로 작성해주세요.`.trim();
  ...
  try {
    ...
    const trimmed = result.trim();
    if (!trimmed || trimmed.length < 10) return defaultComment;
    return safeTruncate(trimmed, 120);
  } catch (err) {
