...
반드시 아래 JSON 형식만 반환하세요:
{
  ...
  "twitter": "X/트위터용 (250자 이내, 해시태그 포함)",
  "instagram": "인스타그램용 (2200자 이내, 감동적인 발견 소식, 해시태그 최대 30개)",
  "general": "일반용 (300자 이내)"
}`;
...
const DEFAULT_PROMO_TEXTS: PlatformPromoTexts = {
  ...
  instagram: '🔍 실종 신고가 접수되었습니다. 목격하신 분은 연락 부탁드립니다. #실종 #찾아주세요 #FindThem',
...
export async function generateThankYouMessage(report: RepostReportInput): Promise<PlatformPromoTexts> {
  ...
  try {
    ...
    return {
      ...
      instagram: `🎉 감사합니다! ${typeLabel} "${report.name}"을(를) 드디어 찾았습니다!\n\n여러분의 도움과 관심이 기적을 만들었습니다. 진심으로 감사드립니다. 🙏\n\n#발견 #FindThem #실종 #찾았습니다`,
