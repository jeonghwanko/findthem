...
체인·토큰 상수는 `@findthem/web3-payment/ui` 서브패스를 사용한다 (viem/CoinGecko 미포함, 번들 사이즈 절감):

```ts
import {
  EVM_TOKENS, EVM_CHAIN_IDS, EVM_CHAIN_META,
  getChainTokenSymbols, getTokenContract,
  type SupportedChainId,
} from '@findthem/web3-payment/ui';

// 체인별 토큰 목록
const symbols = getChainTokenSymbols(chainId);  // ['ETH', 'USDC', 'USDt']

// 토큰 컨트랙트 주소 (네이티브 토큰은 null 반환)
const contract = getTokenContract(chainId, 'USDC');  // 0x...
const native   = getTokenContract(chainId, 'ETH');   // null

// 체인 메타데이터
const { name, nativeCurrency } = EVM_CHAIN_META[chainId];  // 'Ethereum', 'ETH'
```

❌ `@findthem/web3-payment` (메인 진입점) 직접 import 금지 — viem + CoinGecko가 번들에 포함됨.
